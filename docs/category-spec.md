...
- カテゴリ名は空白を許容し、記号は入力時バリデーションで禁止、長さは名称変更時バリデーションで制限する。
- ファイル名に含めるカテゴリ名は、OSで無効な文字（`[\\/:*?"<>|]`）を削除する。
...
## 9. 移行方針
- `schemaVersion` は **1.0.0 のまま**（開発中のため、バージョン変更は最終確定時に行う）。
- 起動時に schemaVersion をチェックし、`1.0.0` の場合は以下を実施する。
...
- マイグレーション処理は開発中の暫定対応とし、最終確定時に見直す。
