...
[2] nc_videosに該当videoIdのスナップショット存在確認
    ├─ 存在 → 既存データを使用（デフォルトでは追加メッセージを表示しない）
    │         比較イベント登録は許可（無言フォールバック）
     └─ 不在 → エラーメッセージ表示
               比較入力を完全抑制
     ↓
...
- オーバーレイでカテゴリを切り替えた場合はページ再読み込み相当として扱い、`lastEventId` / `lastVerdict` をリセットする（既存イベントは移動しない）
...
**役割**: データ閲覧（動画一覧 / 評価一覧）と設定・データ操作を提供する。
