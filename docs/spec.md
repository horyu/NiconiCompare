...
**構成**: タブ形式（動画一覧 / イベント一覧 / 設定 / データ操作）で機能を分割。タブ切替時にフィルタ・ページング状態を sessionStorage に保存し、タブ復帰時に復元する。

1. **評価済み動画一覧タブ**: VideoSnapshot + 最新レーティングのテーブル（ページネーション: 50 件/ページ）。最新レーティングがある動画のみ表示。投稿者（名前）フィルタ／検索、ソート（タイトル / Rating / RD / 評価数 / 勝ち数 / 敗け数 / 最終判定日時）、昇順/降順切替。列は「サムネ / タイトル / 投稿者 / Rating / RD / 評価数 / 勝/分/敗 / 最終判定日時」。サムネに動画リンクを付与。CSV/TSV（CSV は BOM 付きも選択可）でエクスポート可能。
2. **評価イベント一覧タブ**: CompareEvent テーブル（ページネーション: 100 件/ページ）。検索・評価フィルタ（勝ち / 引き分け / 負け）・無効化済み表示の切替・サムネ表示の切替。行アクションで無効化／復活・評価変更・削除（無効化済みのみ）を行う。状態（有効/無効）を含む CSV/TSV エクスポートが可能。詳細なUIは実装を参照。
3. **設定タブ**:
