...
1. **評価済み動画一覧**: VideoSnapshot + 最新レーティングのテーブル（ページネーション: 50 件/ページ）。最新レーティングがある動画のみ表示。投稿者（名前）フィルタ／検索、ソート（Rating / RD / タイトル / 最終判定日時）、昇順/降順切替。列は「サムネ / タイトル / 投稿者 / Rating / RD / 最終判定日時」。サムネに動画リンクを付与。
