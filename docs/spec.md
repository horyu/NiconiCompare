...
- **カテゴリ**は比較の観点。内部的には ID を持つ独立エンティティとして扱う。
- **アクティブカテゴリ**は現在選択されている比較基準で、比較イベント登録・一覧表示の基準になる。
- 1イベント=1カテゴリのみ。自動推定や自動付与は行わない。
- 比較イベント登録時は必ず `activeCategoryId` を `categoryId` として付与する。
- レーティングはカテゴリ単位で更新・集計する。
...
- ユーザーに表示する日時は、画面種別（Options / Popup / 共有HTML）にかかわらず `YYYY/MM/DD HH:mm:ss` 形式で統一し、月・日・時・分・秒は常に 2 桁の 0-padding とする。
- ファイル名に埋め込むタイムスタンプは表示用フォーマットとは分離し、`YYYYMMDDHHmmss` 形式を用いる。

### 9.1 コンテンツオーバーレイ

実視聴ページ上に常駐するコンテンツオーバーレイは「比較操作を中断なく行う」ことだけを目的にし、細かな見た目やアニメーションは実装を正とする。UI の実態は `src/contents/overlay.tsx` を参照し、ここでは変えてはいけない機能要求のみを列挙する。
...
- オーバーレイが閉じた状態では、設定 `showClosedOverlayVerdict` が ON かつ直近評価が存在する場合、`NiconiCompare` の左に `>` `=` `<` のいずれかを表示する。動画または比較対象が変わった場合は表示をクリアする。
- オーバーレイのカテゴリドロップダウンは `overlayVisibleIds` に含まれるカテゴリのみ表示する。
- オーバーレイ展開時はカテゴリドロップダウンの横にオプションページを開く導線（歯車ボタン）を表示し、押下で拡張のオプションページを開ける。
- カテゴリ切替時は `activeCategoryId` を更新し、以後の比較イベントに反映する。
- カテゴリ切替はページ再読み込み相当として扱い、`lastEventId` を明示的にリセットする。既存イベントは移動せず、切替後の新規比較イベントから切替先カテゴリを適用する。
- verdict ボタンの選択状態は同一タブ・同一セッション内で直近に送信した評価のみを表示対象とし、履歴イベントの探索による自動復元は行わない。`currentVideoId` / `opponentVideoId` / `activeCategoryId` が変化した場合は選択状態をクリアする。
...
- アクティブカテゴリの直近イベント一覧を表示（イベント ID と日時、current/opponent のサムネ表示、勝敗は `>` `=` `<` で表記）。
- 見出し行に `勝敗数` チェックボックス（デフォルト OFF）を配置し、ON 時のみ各サムネイル下に `勝/引き分け/敗`（例: `4/1/2`）を表示する。設定値は `nc_settings` に永続化し、次回表示時にも引き継ぐ。
- `overlayAndCaptureEnabled` トグルを提供し、オーバーレイ表示と動画/author 情報の取得を ON/OFF できる（デフォルト ON）。その他の詳細設定は拡張ページへ誘導。
- 見出し行の `NiconiCompare` 右隣にオプションページを開く導線（歯車ボタン）を配置し、押下時はオプションページを開いてポップアップを閉じる。

### 9.3 拡張ページ / Options

**構成**: タブ形式（動画一覧 / イベント一覧 / カテゴリ / 設定 / データ操作）で機能を分割。タブ切替時にフィルタ・ページング状態を sessionStorage に保存し、タブ復帰時に復元する。

1. **評価済み動画一覧タブ**: VideoSnapshot + 最新レーティングのテーブル（ページネーション: 50 件/ページ）。最新レーティングがある動画のみ表示。投稿者（名前）フィルタ／検索、ソート（タイトル / Rating / RD / 評価数 / 勝ち数 / 敗け数 / 最終判定日時）、昇順/降順切替。列は「# / サムネ / タイトル / 投稿者 / Rating / RD / 評価数 / 勝/分/敗 / 最終判定日時」。`#` は表示順に連動する通し番号とする。サムネに動画リンクを付与。評価数セル（0件を除く）はクリック可能で、評価一覧タブへ遷移し該当動画IDで絞り込む。CSV/TSV（CSV は BOM 付きも選択可）でエクスポート可能。カテゴリは表示フィルタとして選択する（`activeCategoryId` は変更しない）。
2. **評価一覧タブ**: CompareEvent テーブル（ページネーション: 100 件/ページ）。検索・評価フィルタ（勝ち / 引き分け / 負け）・無効化済み表示の切替・サムネ表示の切替。行アクションで無効化／復活・評価変更・削除（無効化済みのみ）を行う。状態（有効/無効）を含む CSV/TSV エクスポートが可能。カテゴリは表示フィルタとして選択する（`activeCategoryId` は変更しない）。
3. **カテゴリタブ**:
   - カテゴリ一覧表示（アクティブカテゴリを明示）。
   - 新規追加／名称変更。
   - 削除（移動して削除／破棄して削除）。
   - オーバーレイに表示するカテゴリを選択できる。
   - アクティブカテゴリの切替はオーバーレイのみで行う。
4. **設定タブ**:
   - オーバーレイ自動非表示（ms）、比較候補数、閉じたオーバーレイでの直近勝敗記号表示（デフォルト ON）、Glicko-2 初期値、イベント一覧サムネ表示を編集。Glicko-2 初期値変更時はレーティング再計算、比較候補数変更時は recentWindow を再構築。
   - レーティング再計算ボタン。
5. **データ操作タブ**:
   ...
   - 単一カテゴリの共有用HTMLをエクスポートできる。対象カテゴリはUIで明示選択し、当該カテゴリの有効イベント（`disabled = false`）のみを材料に、オフラインで動作する単体HTML（動画一覧/評価一覧タブ、投稿者名・サムネ・URL、各タブの検索1つ+単一列ソート）を全件出力する。共有HTMLの動画一覧・評価一覧はいずれも先頭に `#` 列を持ち、表示順に連動する通し番号を表示する。
   - 「イベントから辿れない情報を削除」ボタンで `nc_ratings` の孤立 videoId、および参照のない `nc_videos`/`nc_authors` を削除。
...
- **一覧エクスポート**: 評価済み動画一覧と評価一覧から CSV/TSV（CSV は BOM 付きも選択可）を出力できる。
- **共有HTMLエクスポート**: データ操作タブから単一カテゴリの動画一覧+評価一覧を1ファイルHTMLで出力できる。対象は有効イベントのみ、出力は全件、文字コードはUTF-8、JS/CSSはインラインで完全オフライン動作とする。
- **全データ削除**: 設定・履歴・レーティングを含む `nc_*` を初期化し、confirm で確認してから実行。
- `nc_authors`/`nc_videos` の整合性チェックは行わないが、欠損は UI で明示。
- クリーンアップ実装:
  - `chrome.alarms` で24時間ごとに発火し、`runAutoCleanupIfNeeded` が `nc_meta.lastCleanupAt` を参照して24時間以上経過している場合のみ実行する。参照されない `nc_ratings`/`nc_videos`/`nc_authors` を削除する（評価済みイベントで参照されている動画、現在再生中の動画、recentWindow に載っている動画、固定中の比較対象は保持）。最終実行時刻は `nc_meta.lastCleanupAt` に保存する。ユーザーが手動でクリーンアップボタンを押した場合も同じ処理を実行する。
