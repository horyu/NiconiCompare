...
- **比較候補 Select**:
  ...
  - 各候補のラベルは `動画タイトル | 動画ID` 形式とし、単なる ID ではなく判別しやすい情報を提供する。
  - カード内では 3 列（左=再生中、中央=vs/引き分け、右=選択中）の固定グリッドを採用し、左と右にサムネイル＋タイトル、中央列に `vs` と引き分けボタンを配置する。右列 3 行目は「動画 ID ラベル + タイトル表示 + 透明な Select」を縦積みにし、Select の option は常に `動画ID | 動画タイトル` 形式とする。
...
- **カードサイズとビューポート**: カード幅 320px をベースに最大 555px まで拡張可能（高さは最大 400px）。各評価ボタンは幅 105px 固定。ビューポート外にはみ出す場合は位置を自動調整（右端 → 左側に表示、下端 → 上に配置）。
...
3. **Options 設定**:
   - LRU サイズ（1〜10）や Glicko-2 初期値等を編集（オーバーレイ位置は固定、デフォルトの自動閉鎖待機時間は 2 秒で必要に応じて設定可能）。変更後はイベントログをリプレイ。
