...
- **常駐/展開**: 右上のミニアイコン（24×24px、半透明背景）をホバーするとカードが展開。マウスアウト後のカード自動閉鎖タイムアウト（デフォルト 1 秒、Options で 0.5〜5 秒に設定可）で閉じる（再ホバーでリセット）。Popup から `overlayEnabled` が OFF の場合はアイコン自体を表示せず、動画・author 情報の取得も行わない。
