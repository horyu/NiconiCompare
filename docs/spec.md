...
1. **評価済み動画一覧タブ**: VideoSnapshot + 最新レーティングのテーブル（ページネーション: 50 件/ページ）。最新レーティングがある動画のみ表示。投稿者（名前）フィルタ／検索、ソート（タイトル / Rating / RD / 評価数 / 勝ち数 / 敗け数 / 最終判定日時）、昇順/降順切替。列は「サムネ / タイトル / 投稿者 / Rating / RD / 評価数 / 勝/分/敗 / 最終判定日時」。サムネに動画リンクを付与。CSV/TSV（CSV は BOM 付きも選択可）でエクスポート可能。カテゴリは表示フィルタとして選択する（`activeCategoryId` は変更しない）。
...
5. **データ操作タブ**:
   ...
   - 単一カテゴリの共有用HTMLをエクスポートできる。対象カテゴリはUIで明示選択し、当該カテゴリの有効イベント（`disabled = false`）のみを材料に、オフラインで動作する単体HTML（動画一覧/評価一覧タブ、投稿者名・サムネ・URL、各タブの検索1つ+単一列ソート）を全件出力する。
