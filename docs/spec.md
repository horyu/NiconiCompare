...
5. **データ操作タブ**:
   ...
   - 単一カテゴリの共有用HTMLをエクスポートできる。対象カテゴリはUIで明示選択し、当該カテゴリの有効イベント（`disabled = false`）のみを材料に、オフラインで動作する単体HTML（動画一覧/評価一覧タブ、投稿者名・サムネ・URL、各タブの検索1つ+単一列ソート）を全件出力する。共有HTMLの動画一覧は先頭に `#` 列を持ち、表示順に連動する通し番号を表示する。
