...
- 対応済み。
- `messages.ts` に message type ごとの success response map と `MessageResponse<TMessage>` を追加。
- `sendNcMessage` は渡した message から戻り値型を推論するように変更。
- `runNcAction` も action のレスポンス型を保持するように変更。
- `requestState` の snapshot 型を `StateSnapshot` として公開し、Options / Overlay の手動 generic 指定を削除。
- `messages.test.ts` に `recordEvent`、`requestState`、`updateSettings` の型推論テストを追加。
...
- 対応済み。
- `buildEventMutationUpdates` に `events` 変更検知と `ratings` 再構築を集約。
- イベント未変更時は `ratings` を再構築せず、追加 updates のみ返す。
- 既存イベントの評価変更、イベント無効化/復活/完全削除、カテゴリ削除/一括移動で共通 helper を使用。
...
- 対応済み。
- Events / Videos の復元値を項目ごとに正規化し、不正な値だけ既定値に戻すようにした。
- `page` は正の整数、`verdict`、`sort`、`order` は選択肢の許可値だけを受け入れる。
- `src/options/utils/sessionStorage.test.ts` と `sessionState.test.ts` に破損 JSON と不正な復元値の回帰テストを追加。
...
### 2026-07-18: sessionStorage 復元値の検証

- 変更方針: JSON を型 assertion せず、Events / Videos の状態をフィールド単位で正規化する。
